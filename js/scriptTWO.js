...
async function buildQuiz() {
  let response = await fetch(api);
  questionBox = await response.json();
  // нам понадобится место для хранения вывода HTML
  const output = []
  // for each question...
  questionBox.results.forEach((currentQuestion, questionNumber) => {
    currentQuestion.incorrect_answers = currentQuestion.incorrect_answers.concat(currentQuestion.correct_answer);
    // return elem
...
const checkResult = (e) => {
  ...
  if (tar.tagName === 'INPUT') {
    ...
    const isCorrect = questionBox[questionNumber].correct_answer === userAnswer;
